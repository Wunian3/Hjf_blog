...
func (ApiBigModel) RoleUpdateView(c *gin.Context) {
	...
	for key, mp := range maps {
		...
		if key == "name" {
			...
			if err == nil {
				res.FailWithMessage("角色名称和已有的角色名称重复", c)
				return
			}
		}
	}
	if len(modelMap) == 0 {
		res.OkWithMessage("角色数据暂无更新", c)
		return
	}
	err = global.DB.Model(&arm).Updates(modelMap).Error
	if err != nil {
		logrus.Error(err)
		res.FailWithMessage("角色更新失败", c)
		return
	}

	res.OkWithMessage("角色更新成功", c)
}

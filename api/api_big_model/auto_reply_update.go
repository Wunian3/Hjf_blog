...
func (ApiBigModel) AutoReplyUpdateView(c *gin.Context) {
	...
	if cr.Mode == 4 {
		...
		if err != nil {
			res.FailWithMessage(fmt.Sprintf("正则表达式错误 %s", err.Error()), c)
			return
		}
	}
	if cr.ID == 0 {
		...
		if err == nil {
			res.FailWithMessage("规则名称不能相同", c)
			return
		}
		...
		if err != nil {
			...
			res.FailWithMessage("数据添加失败", c)
			return
		}
		res.OkWithMessage("自动回复添加成功", c)
		return
	}
	var arm models.AutoReplyModel
	...
	if err == nil {
		res.FailWithMessage("和已有的规则名称重复", c)
		return
	}
